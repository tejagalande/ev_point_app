...
//   doesMapReady() {
//     !_mapReady ?  _mapReady = true : null ;
//     notifyListeners();
//   }

//   Future<void> getCurrentLocationAndMove() async {
//   if (!_mapReady || _completer == null) return;
...
void initializeMarkers() {
  ...
  void onMapCreated(GoogleMapController controller) async {
    ...
    if(!_isDisposed) { log("called _handleLocationProcess func ");  _handleLocationProcess();}
    });
    
    // Stage 3: Enable essential features
    _featuresTimer = Timer(Duration(milliseconds: 1200), () {
    if(!_isDisposed)  {log("called _enableEssentialFeatures func "); _enableEssentialFeatures();}
    });
  }
  ...
  Future<void> _getCurrentLocationAndAnimate() async {
    if (!_mapReady || _controller == null || _isDisposed || _completer == null) {
    ...
    try {
      ...
      if (_isDisposed || _controller == null || _completer == null || _completer!.isCompleted == false) {
        return;       
      }
  ...
  void onMapInteraction() {
    ...
    log("initial values: map ready=$_mapReady && lite mode enable=$_liteModeEnable");
    if ( _liteModeEnable) {
      _liteModeEnable = false;
